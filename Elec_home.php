...
<?php
$today=date("Y-m-d");
$query="SELECT time, SUM(current_usage) FROM elec_usage WHERE date=:today GROUP BY time";
...
{
  $chart_data .= "{ time:'".$record["time"]."', current_usage:".$record["SUM(current_usage)"]."}, ";
  $current_flow+=$record["SUM(current_usage)"];
}
$energy=$current_flow/1000;
$units=$energy/3600;
$chart_data = substr($chart_data, 0, -2);
$db=new PDO('mysql:host=localhost;dbname=userinfo','root','');
$db->setAttribute(PDO::ATTR_ERRMODE,PDO::ERRMODE_EXCEPTION);
$q="SELECT COUNT(username) FROM userinfo WHERE Elecaccount_no!=''";
$sth=$db->query($q);
$count_elec=$sth->fetchColumn();
    ...
    <div id="webdash">
           ...
           <br>
              ...
              <div class="col-12 col-md-10">
                ...
                <div class="card col-lg-7 col-11" style="margin:20px;height:50%">
                <div class="card-header">
                <h5 class="card-title">Realtime Electricity Usage <?php echo $today ?></h5>
                <p class="card-title lead" style="margin:0">Total Usage(KW) : <?php echo $energy ?></p>
                <p class="card-title lead">Total Usage(units) : <?php echo $units ?></p>
                </div>
                <div class="card-body">

                  <div id="chart" style="color:gray">
                    <script>
                    Morris.Area({
                     element : 'chart',
                ...
                <div class="card-body">
                <p class="card-title lead text-center"><?php echo " CEB SmartMeter" ?></p>
                <p class="card-title lead text-center"><?php echo $count_elec." Users" ?></p>
                </div>
                </div>
                <!--------------------------------------------------------->
